<script lang="ts">
	 ...
	 * Triggers conversation branching when a preset is selected.
	 */
	...
	async function handleSelect(id: string | null): Promise<void> {
		if (isApplying) return;
		isApplying = true;
		try {
			await chatStore.applyPresetToConversation(id);
			if (id) {
